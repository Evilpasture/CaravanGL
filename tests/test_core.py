...
def test_pipeline_render_state_persistence():
    ...
    pipe = caravangl.Pipeline(
        program=prog, vao=vao, depth_test=1, depth_write=0, blend=1
    )
...
def test_full_pipeline_draw():
    ...
    verts = np.array(
        [-0.5, -0.5, 0, 0, 0, 0.5, -0.5, 0, 1, 0, 0, 0.5, 0, 0.5, 1], dtype=np.float32
    )
...
def test_indexed_draw_path():
    ...
    ibo = caravangl.Buffer(
        size=len(i_data), data=i_data, target=caravangl.ELEMENT_ARRAY_BUFFER
    )
    vao.bind_index_buffer(ibo)

    # Create indexed pipeline
    pipe = caravangl.Pipeline(program=prog, vao=vao, index_type=caravangl.UNSIGNED_INT)
